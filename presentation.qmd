...
## What one PhD student found

\doublestat{-0.1\%}{average growth of high-debt countries, as published}{+2.2\%}{after one graduate student fixed the spreadsheet}

\centering

A famous 2010 austerity result rested on an Excel formula that skipped five countries. Thomas Herndon caught it in a class replication assignment.

\source{Herndon, Ash, and Pollin (2014), Cambridge Journal of Economics}
...
\takeaway{Anyone should be able to delete everything except the raw data and the code, and rebuild your whole project.}
...
You run "*make*" in the CLI to rebuild everything, or a single step with "*make r*", "*make stata*", "*make paper*", or "*make slides*" ![](images/paste-16.png){fig-align="center" height="125"}
...
\takeaway{One pipeline, three outputs (report, paper, slides), and the numbers always in sync.}
...
-   Agent: reads your files, edits code, runs commands
-   It iterates until the pipeline runs
-   Examples: Claude Code, OpenAI Codex, GitHub Copilot, Cursor
:::
...
-   Drafting analysis code
-   Explaining errors
-   Converting between Stata, R, and Quarto
-   Polishing prose
:::
...
-   Literature claims and citations (it invents them)
...
\takeaway{Use agents where output is checkable. Your judgment is not delegable.}

\source{Korinek (2023, 2025); de Kok (2025)}

::: notes
The pattern behind the two columns is checkability. Reshaping a panel dataset is checkable: the code either reproduces the numbers or it does not. Whether year fixed effects are the right design choice is judgment, and a model will happily argue either side. Citations are the classic trap, because models invent plausible references with real journal names. Korinek maps these use cases for economists, and de Kok shows what careful validation looks like for textual analysis in our field. The rule of thumb: we can delegate generation, we can never delegate verification.
:::

## 4. AI agents: What the evidence shows

\doublestat{24\%}{expected speed-up from AI}{19\%}{measured slowdown in a randomized trial}

-   On well-scoped tasks, AI made developers 56% faster; on their own mature code, slower
-   The best agent reproduced 78% of papers in a 2026 benchmark; nudged to "match the paper", it often fabricated instead of saying no
-   Generation is cheap; verification is your job

\source{METR (2025); Peng et al. (2023); Alizadeh et al. (2026)}

::: notes
The number pair comes from a 2025 randomized trial by METR: sixteen experienced developers, 246 real tasks, in their own mature repositories. They expected AI to make them 24 percent faster. Measured, they were 19 percent slower, and afterwards they still believed they had been faster. An earlier Copilot trial found the opposite on a small, well-scoped task: 56 percent faster. The difference is the task, not the tool, and your dissertation repo is the mature-codebase case. The 2026 benchmark is encouraging and sobering at once. Claude Code reproduced 78 percent of social science papers end to end. But when nudged to match the paper's numbers, it often fabricated plausible results instead of reporting failure. Trust comes from verification, not from confidence.
:::
...
-   Prompts nobody saved; models get retired, chats cannot be rerun
-   Roughly one in three articles not reproducible, even with disclosure [@FGHKO_2023]

\bigbreak

\highlightbox{A US court fined two lawyers \$5{,}000 for citing six cases ChatGPT had invented. Asked to double-check, ChatGPT vouched for its own fakes (Mata v. Avianca, 2023).}

::: notes
Dark use means AI is helping, but none of that help lives in the project. A result gets copied from a chat window straight into a draft. An agent edits a file and nobody looks at what changed. The prompt that produced a number sits in a chat history nobody saved. And chats age badly: providers retire models, so a chat-based step can become unrunnable while code stays runnable. Fišar et al. found that roughly a third of Management Science articles could not be reproduced, even under a strict disclosure policy, before agents were part of the picture. Add an unrecorded chat on top of that, and reproducibility gets worse, not better. The court case in the box is what no-verification looks like: the lawyers asked the same chat window to check its own work, and it vouched for its own fabrications. When a committee or a journal asks us to defend a number, "the AI produced it" is not an answer they will accept, and it should not be one we accept either.
:::

## 4. AI agents: Your pipeline is the guardrail

::::: columns
::: {.column width="52%"}
-   **Numbered folders** scope what the agent may touch
-   "*0_data*" stays read-only: the one hard rule
-   **git** turns every agent edit into a reviewable diff
-   **make** verifies every number with one command
-   **README + AGENTS.md** are the agent's onboarding memo
:::
...
## 4. AI agents: What leaves your machine, and what do you disclose?

::::: columns
::: {.column width="50%"}
**Before you paste**

-   Assume consumer chat apps train on and keep what you paste
-   Keep licensed data (WRDS, Compustat) and confidential data out; check the license first
-   Use institutional or API access, or a local model, for sensitive projects
:::

::: {.column width="50%"}
**Before you submit**

-   Disclose GenAI use: *CAR* wants it described "transparently and in detail" in Methods
-   Never list AI as an author; the responsibility stays yours
-   Log model, prompt, and validation: an AI Use Statement in your README
:::
:::::

\source{CAR editorial policies (2024); ICMJE/COPE; World Bank, Documenting AI Use for Reproducible Research (2026)}

::: notes
Two checklists, one for data going out, one for text coming in. On privacy, the default settings of consumer chat apps can use our pastes for training and keep them for years, while API and institutional tiers do not train by default, and a local model sends nothing at all. Licensed databases like WRDS or Compustat restrict redistribution, so check the license before an extract goes anywhere. At a European school, GDPR adds its own transfer rules for personal data. On disclosure, our own journals moved first: CAR has required a declaration since 2024, and management journals are broadening what counts as disclosable AI use. Management Science also bars reviewers from uploading manuscripts into these tools, so confidentiality cuts both ways. The last line is the cheapest insurance: keep the model name, the prompt, and how you validated the output with the project, and your methods section writes itself.
:::
...
-   Numbers from a chat window into a draft
-   Confidential or licensed data into a chat
-   Undisclosed AI use in a submission
-   Hand-edits to "*3_output*", by you or the agent
:::
:::::
...
::: notes
Three things to take home. First, AI used in the dark, chat windows and untracked edits, produces numbers nobody can defend. Second, the pipeline we built today is exactly what keeps an agent transparent: it works in files, under git, against make. Third, the routine is short. We read the diff before we commit, we rerun the pipeline before we trust a number, and we disclose the use when we submit. Two honest caveats. Output is often wrong, confidently wrong, so verification is not optional. And skipping AI entirely remains a valid way to do a PhD. When the agent added our winsorizing step, the diff showed one changed file in 1_code, and a clean make rebuilt the same table in the draft. That is what owning an AI-assisted result looks like.
:::
