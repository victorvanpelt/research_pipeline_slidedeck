...
## 4. AI agents: Two ground rules before any tool
...
-   Ground rule 2: every person has their own solution
    -   Coauthors bring different agents; the shared repo stays agent-agnostic
-   The one agent file that ships: "*AGENTS.md*", tool-neutral rules for all
...
::: notes
Before any tool, two ground rules. First, agentic coding is evolving rapidly. Any tool-specific setup we ship today might be obsolete within a week, so the repository ships none. Second, every person has their own solution. One coauthor uses one vendor's agent, the next another's, a third uses none at all, and all of these are fine. So nothing in the shared repository may depend on anyone's choice. The one agent file that does ship is AGENTS.md, tool-neutral by design and read by more than 20 different tools. We return to that file in a few slides. Think of it this way: your agent setup is personal, like your editor. The rules it follows are shared, like the folder structure and the makefile.
:::
