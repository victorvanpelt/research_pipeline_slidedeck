...
\begin{frame}{What one PhD student found}
\phantomsection\label{what-one-phd-student-found}
\doublestat{-0.1\%}{average growth of high-debt countries, as published}{+2.2\%}{after one graduate student fixed the spreadsheet}

\centering

A famous 2010 austerity result rested on an Excel formula that skipped
five countries. Thomas Herndon caught it in a class replication
assignment.
...
paper}'', or ``\emph{make slides}'' \begin{center}
...
what turns a chatbot into a collaborator inside the pipeline. One
caution: these tools change fast. Google already renamed and merged its
own coding agent this year, so write down which tool and model you used,
and expect the names to change.}
\end{frame}

\begin{frame}{4. AI agents: What can an agent do for your research?}
\phantomsection\label{ai-agents-what-can-an-agent-do-for-your-research}
\begin{columns}[T]
...
\note{The pattern behind the two columns is checkability. Reshaping a
panel dataset is checkable: the code either reproduces the numbers or it
does not. Whether year fixed effects are the right design choice is
judgment, and a model will happily argue either side. Citations are the
classic trap, because models invent plausible references with real
journal names. Korinek maps these use cases for economists, and de Kok
shows what careful validation looks like for textual analysis in our
field. The rule of thumb: we can delegate generation, we can never
delegate verification.}
\end{frame}

\begin{frame}{4. AI agents: What the evidence shows}
\phantomsection\label{ai-agents-what-the-evidence-shows}
\doublestat{24\%}{expected speed-up from AI}{19\%}{measured slowdown in a randomized trial}

\begin{itemize}
\tightlist
\item
  On well-scoped tasks, AI made developers 56\% faster; on their own
  mature code, slower
\item
  The best agent reproduced 78\% of papers in a 2026 benchmark; nudged
  to ``match the paper'', it often fabricated instead of saying no
\item
...
\note{The number pair comes from a 2025 randomized trial by METR:
sixteen experienced developers, 246 real tasks, in their own mature
repositories. They expected AI to make them 24 percent faster. Measured,
they were 19 percent slower, and afterwards they still believed they had
been faster. An earlier Copilot trial found the opposite on a small,
well-scoped task: 56 percent faster. The difference is the task, not the
tool, and your dissertation repo is the mature-codebase case. The 2026
benchmark is encouraging and sobering at once. Claude Code reproduced 78
percent of social science papers end to end. But when nudged to match
the paper's numbers, it often fabricated plausible results instead of
reporting failure. Trust comes from verification, not from confidence.}
\end{frame}
...
produced a number sits in a chat history nobody saved. And chats age
badly: providers retire models, so a chat-based step can become
unrunnable while code stays runnable. Fišar et al.~found that roughly a
third of Management Science articles could not be reproduced, even under
a strict disclosure policy, before agents were part of the picture. Add
an unrecorded chat on top of that, and reproducibility gets worse, not
better. The court case in the box is what no-verification looks like:
the lawyers asked the same chat window to check its own work, and it
vouched for its own fabrications. When a committee or a journal asks us
to defend a number, ``the AI produced it'' is not an answer they will
accept, and it should not be one we accept either.}
\end{frame}

\begin{frame}{4. AI agents: Your pipeline is the guardrail}
\phantomsection\label{ai-agents-your-pipeline-is-the-guardrail}
\begin{columns}[T]
\begin{column}{0.52\linewidth}
\begin{itemize}
\tightlist
\item
  \textbf{Numbered folders} scope what the agent may touch
\item
  ``\emph{0\_data}'' stays read-only: the one hard rule
\item
  \textbf{git} turns every agent edit into a reviewable diff
\item
  \textbf{make} verifies every number with one command
\item
  \textbf{README + AGENTS.md} are the agent's onboarding memo
\end{itemize}
\end{column}
...
data to draft, the result is verified, not just plausible. The file on
the right is real and now ships with the example repository. AGENTS.md
is ``a README for agents'': an open standard since 2025, used in over
sixty thousand repositories and read by most major tools, while Claude
Code reads a CLAUDE.md. It gives the agent the same onboarding a new
collaborator would need. None of this requires trusting the agent's
judgment, it requires trusting the guardrails we already built.}
\end{frame}
...
window, all of it comes from code we can rerun.}
\end{frame}

\begin{frame}{4. AI agents: What leaves your machine, and what do you
disclose?}
\phantomsection\label{ai-agents-what-leaves-your-machine-and-what-do-you-disclose}
\begin{columns}[T]
\begin{column}{0.5\linewidth}
\textbf{Before you paste}
...
\item
  Keep licensed data (WRDS, Compustat) and confidential data out; check
  the license first
\item
  Use institutional or API access, or a local model, for sensitive
  projects
\end{itemize}
\end{column}

\begin{column}{0.5\linewidth}
\textbf{Before you submit}

\begin{itemize}
\tightlist
\item
  Disclose GenAI use: \emph{CAR} wants it described ``transparently and
...
\note{Two checklists, one for data going out, one for text coming in. On
privacy, the default settings of consumer chat apps can use our pastes
for training and keep them for years, while API and institutional tiers
do not train by default, and a local model sends nothing at all.
Licensed databases like WRDS or Compustat restrict redistribution, so
check the license before an extract goes anywhere. At a European school,
GDPR adds its own transfer rules for personal data. On disclosure, our
own journals moved first: CAR has required a declaration since 2024, and
management journals are broadening what counts as disclosable AI use.
Management Science also bars reviewers from uploading manuscripts into
these tools, so confidentiality cuts both ways. The last line is the
cheapest insurance: keep the model name, the prompt, and how you
validated the output with the project, and your methods section writes
itself.}
\end{frame}
...
\item
  Keep ``\emph{0\_data}'' read-only, always
\item
...
\item
  Hand-edits to ``\emph{3\_output}'', by you or the agent
\end{itemize}
\end{column}
\end{columns}
...
We read the diff before we commit, we rerun the pipeline before we trust
a number, and we disclose the use when we submit. Two honest caveats.
Output is often wrong, confidently wrong, so verification is not
optional. And skipping AI entirely remains a valid way to do a PhD. When
the agent added our winsorizing step, the diff showed one changed file
in 1\_code, and a clean make rebuilt the same table in the draft. That
is what owning an AI-assisted result looks like.}
\end{frame}
