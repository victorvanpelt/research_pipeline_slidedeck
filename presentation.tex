...
enter the repo, the same environment-variable rule from chapter one. The
Build section hands the agent the make targets, so it can check its own
work. And the last rule bans git commit and git push: the agent leaves
the diff for us. The next three slides are these rules in action.}
\end{frame}
