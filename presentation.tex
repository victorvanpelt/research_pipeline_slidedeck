...
\tightlist
\item
  An agent can hand you a merged dataset, a finished table, a perfect
  figure
\item
  The problem: no code behind it. You cannot rerun it, check it, or fix
  it
\item
...
\highlightbox{Code the agent wrote yesterday still runs in ten years. The model that probablistically wrote it will not.}
\end{frame}
...
\begin{frame}{4. AI agents: Set the rules before it starts}
\phantomsection\label{ai-agents-set-the-rules-before-it-starts}
\begin{columns}[T]
...
the right is real and ships with the example repository. AGENTS.md is a
README for agents, a convention most coding agents read, and Claude Code
reads a CLAUDE.md. It gives the agent the same onboarding a new
...
window, all of it comes from code we can rerun. One more reason the
rerun matters: agents are built to please, and pushed for a result they
will sooner produce a plausible number than admit failure. Our own make
run is the one test that cannot be flattered.}
\end{frame}
...
\item
  Bigger changes: let the agent work on a branch, review it as a pull
  request
\item
...
keep the trail in the commit history. Two lines deserve a sentence each.
